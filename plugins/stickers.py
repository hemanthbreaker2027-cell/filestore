...
async def setup_stickers(client: Bot, message: Message):
    try:
        ...
        if ask_count.text.lower() == "/cancel":
             return await message.reply("❌ Setup cancelled.")
        count = int(ask_count.text)
    except ValueError:
        return await message.reply("❌ Invalid number provided. Please enter an integer.")
    except asyncio.TimeoutError:
        return await message.reply("❌ Setup timed out (60s). Please try again.")
    except Exception as e:
        print(f"Sticker Setup Error (Count): {e}")
        return await message.reply(f"❌ Unexpected Error: {e}")

    for i in range(1, count + 1):
        try:
            # Get Name
            ask_name = await client.ask(message.chat.id, f"<b>Enter Name #{i}:</b>\n\n(Send /cancel to stop)", filters=filters.text, timeout=60)
            if ask_name.text.lower() == "/cancel":
                await message.reply("❌ Setup stopped.")
                break
            keyword = ask_name.text.strip()

            # Get Sticker
            ask_sticker = await client.ask(message.chat.id, f"<b>Send Sticker for '{keyword}':</b>\n\n(Send /cancel to stop)", timeout=60)
            if ask_sticker.text and ask_sticker.text.lower() == "/cancel":
                 await message.reply("❌ Setup stopped.")
                 break

            if not ask_sticker.sticker:
                 await message.reply("❌ That's not a sticker! Skipping this one.")
                 continue
            ...
            await message.reply(f"❌ Timed out waiting for input #{i}.")
            break
        except Exception as e:
            print(f"Sticker Setup Error at step {i}: {e}")
